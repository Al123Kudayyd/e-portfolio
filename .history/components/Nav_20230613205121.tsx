      ...
      <div className='py-5 mx-10 flex items-center justify-end font-bold cursor-pointer transition duration-300'>
        <FaBars onClick={() => setModal(true)} className='flex md:hidden lg:hidden text-2xl hover:text-white transition duration-300' />
        {
          ...
          <div className='overscroll-y-none'>
            ...
            <div className=' w-screen h-screen md:hidden p-0 m-0 z-50 flex flex-col items-center justify-center'>  
              <ul className='space-y-16 text-white text-2xl'>
                <li onClick={() => setModal(false)} className='hover:text-black transition duration-300'><a href="#about">About me</a></li>
                <li onClick={() => setModal(false)} className='hover:text-black transition duration-300'><a href="#stack"> My Skills</a></li>
                <li onClick={() => setModal(false)} className='hover:text-black transition duration-300'><a href="#projects"> Projects</a></li>
                <li onClick={() => setModal(false)} className='hover:text-black transition duration-300'><a href="#contact"> Contact</a></li>
              </ul>
            </div>
          </div>
        }
        <ul className='md:flex dark:text-white lg:flex hidden space-x-5 items-center justify-end text-xs sm:text-xl'>
            ...
            {
              darkMode ? <li onClick={() => setDarkMode(false)}><MdOutlineModeNight onClick={handleCLick} className='text-3xl hover:scale-110 transition' /></li> : <li><MdOutlineModeNight onClick={() => changeMode()} className='text-3xl hover:scale-110 transition' /></li>
            }
        </ul>
      </div>
    </section>
